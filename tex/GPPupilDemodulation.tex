...
\begin{equation}
    w_k = \Var\left(\Abs{d_m}\right)_{m\in \textrm{ST}(k)}
\end{equation}
...
\end{align}
To take into an account this precision, we can rewrite this equation as:
...
\end{align}
where $\div$ is the element-wise division and 
\begin{equation}
    \V w' =  \V w \times \sqrt{\V p}
\end{equation}
...
As seen on figure \ref{fig:faintzoom}, there is a lag of about 10ms between the switching of voltage and the actual dimming/brightening of the laser. This lag is automatically estimated and corrected in the Julia code. 
