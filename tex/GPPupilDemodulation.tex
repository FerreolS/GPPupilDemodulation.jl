...
In that case, the equation~\ref{eq:r} solved to retrieve the modulation parameters can be updated   as:
\begin{align}
    (c^+, a^+, b^+, \phi^+) &= \argmin_{c,a,b,\phi} \Norm{\V{r} -  c -  \V{p} \times a \exp\left( \jmath \, b \sin\left( \V{\omega} + \phi\right)\right) }_{\V{w}}^2
\end{align}
...
\begin{equation}
    ...
    a^+
    \end{array}\right] =\left[\begin{array}{ll}
        \sum_i w_i & \sum_i w_i\, \, p_i\,m_i\\
        \sum_i   w_i\, p_i\,m_i^* &  \sum_i w_i\, \abs{p_i\,m_i}^2
        \end{array}\right]^{-1} \cdot
    \left[\begin{array}{l}
    \sum_i w_i\,d_i\\
    \sum_i w'i\, p_i\,m_i^*\,d_i\\
    \end{array}\right] 
\end{equation}
